...
class ImGuiFileBrowser
{
  public:
    ...
    /* Use this to show an open file dialog. The function takes label for the
     * window, the size, a DialogMode enum value defining in which mode the
     * dialog should operate and optionally the extensions that are valid for
     * opening. Note that the select directory mode doesn't need any extensions.
     */
    ...
    /* Store the opened/saved file name or dir name (incase of
     * selectDirectoryDialog) and the absolute path to the selection Should only
     * be accessed when above functions return true else may contain garbage.
     */
  ...
  private:
     ...
     * and help in validating file extensions and for filtering, parsing top
     * navigation bar.
     */
    ...
    // Functions that reset state and/or clear file list when reading new
    // directory
    void clearFileList();
    void closeDialog();
    ...
    bool
    loadWindowsDrives(); // Helper Function for Windows to load Drive Letters.
#endif

#if defined(unix) || defined(__unix__) || defined(__unix) || defined(__APPLE__)
    void initCurrentPath(); // Helper function for UNIX based system to load
                            // Absolute path using realpath
#endif
    ...
    bool       show_hidden, show_inputbar_combobox, is_dir, is_appearing,
      filter_dirty, validate_file;
    char input_fn[256];
    ...
    std::string current_path, error_msg, error_title, invfile_modal_id,
      repfile_modal_id;

    ImGuiTextFilter filter;
    std::string     valid_types;
    std::vector<const Info*>
      filtered_dirs; // Note: We don't need to call delete. It's just for
                     // storing filtered items from subdirs and subfiles so we
                     // don't use PassFilter every frame.
