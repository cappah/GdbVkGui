...
void
ImGuiFileBrowser::clearFileList()
{
...
bool
ImGuiFileBrowser::showFileDialog(const std::string& label,
                                 ...
                                 const std::string& valid_types)
{
    ...
    ImGui::SetNextWindowPos(
      io.DisplaySize * 0.5f, ImGuiCond_Appearing, ImVec2(0.5f, 0.5f));
    ImGui::SetNextWindowSize(
      ImVec2(std::max(sz_xy.x, min_size.x), std::max(sz_xy.y, min_size.y)),
      ImGuiCond_Appearing);
    ...
    if (ImGui::BeginPopupModal(label.c_str(),
        ...
        // If this is the initial run, read current directory and load data
        // once.
        if (is_appearing) {
            selected_fn.clear();
            selected_path.clear();
            ...
            /* If current path is empty (can happen on Windows if user closes
             * dialog while inside MyComputer. Since this is a virtual folder,
             * path would be empty) load the drives on Windows else initialize
             * the current path on Unix.
             */
        ...
        if (validate_file) {
            ...
            if (!check && dialog_mode == DialogMode::OPEN) {
                ...
                error_msg = "Invalid Directory Selected. Please make sure the "
                            "directory exists.";
            }

            // If selected file passes through validation check, set path to the
            // file and close file dialog
            if (check) {
                selected_path = current_path + selected_fn;

                // Add a trailing "/" to emphasize its a directory not a file.
                // If you want just the dir name it's accessible through
                // "selected_fn"
        ...
        // We don't need to check as the modals will only be shown if OpenPopup
        // is called
...
bool
ImGuiFileBrowser::renderNavAndSearchBarRegion()
{
    ...
    ImVec2 sw_size         = ImVec2(ImGui::CalcTextSize("Random").x + 140,
                            style.WindowPadding.y * 2.0f + frame_height);
    ImVec2 sw_content_size = sw_size - style.WindowPadding * 2.0;
    ImVec2 nw_size =
      ImVec2(pw_content_size.x - style.ItemSpacing.x - sw_size.x, sw_size.y);

    ImGui::BeginChild("##NavigationWindow",
                      nw_size,
                      true,
    ...
    for (int i = 0; i < current_dirlist.size(); i++) {
        ...
        if (i != current_dirlist.size() - 1) {
            ImGui::SameLine(0, 0);
            float next_label_width =
              ImGui::CalcTextSize(current_dirlist[i + 1].c_str()).x;
            ...
            if (ImGui::GetCursorPosX() + next_label_width >=
                ...
                if (ImGui::BeginPopup("##NavBarDropboxPopup")) {
                    ...
                    if (ImGui::ListBoxHeader("##NavBarDropBox",
                                             ImVec2(0, list_item_height * 5))) {
                        ImGui::PushStyleColor(
                          ImGuiCol_Text, ImVec4(0.882f, 0.745f, 0.078f, 1.0f));
                        for (int j = i + 1; j < current_dirlist.size(); j++) {
                            if (ImGui::Selectable(current_dirlist[j].c_str(),
                                                  false) &&
    ...
    if (filter.Draw("##SearchBar", sw_content_size.x - marker_width) ||
        filter_dirty)
...
ImGuiFileBrowser::renderFileListRegion()
{
    ...
    float input_bar_ypos   = pw_size.y -
                           ImGui::GetFrameHeightWithSpacing() * 2.5f -
                           style.WindowPadding.y;
    float window_height =
      input_bar_ypos - ImGui::GetCursorPosY() - style.ItemSpacing.y;
    ...
    // Reinitialize the limit on number of selectables in one column based on
    // height
    col_items_limit = static_cast<int>(
      std::max(1.0f, window_content_height / list_item_height));
    int num_cols = static_cast<int>(
      std::max(1.0f,
               std::ceil(static_cast<float>(filtered_dirs.size() +
                                            filtered_files.size()) /
                         col_items_limit)));

    // Limitation by ImGUI in 1.75. If columns are greater than 64 readjust the
    // limit on items per column and recalculate number of columns
    if (num_cols > 64) {
        int exceed_items_amount = (num_cols - 64) * col_items_limit;
        col_items_limit +=
          static_cast<int>(std::ceil(exceed_items_amount / 64.0));
        num_cols = static_cast<int>(
          std::max(1.0f,
                   std::ceil(static_cast<float>(filtered_dirs.size() +
                                                filtered_files.size()) /
                             col_items_limit)));
    ...
    for (int i = 0; i < filtered_dirs.size(); i++) {
        if (!filtered_dirs[i]->is_hidden || show_hidden) {
            ...
            if (ImGui::Selectable(filtered_dirs[i]->name.c_str(),
                                  selected_idx == i && is_dir,
                                  ImGuiSelectableFlags_AllowDoubleClick)) {
                selected_idx = i;
                is_dir       = true;

                // If dialog mode is SELECT then copy the selected dir name to
                // the input text bar
                if (dialog_mode == DialogMode::SELECT)
    ...
    for (int i = 0; i < filtered_files.size(); i++) {
        if (!filtered_files[i]->is_hidden || show_hidden) {
            ...
            if (ImGui::Selectable(filtered_files[i]->name.c_str(),
                ...
                // If dialog mode is OPEN/SAVE then copy the selected file name
                // to the input text bar
...
bool
ImGuiFileBrowser::renderInputTextAndExtRegion()
{
    std::string label =
      (dialog_mode == DialogMode::SAVE) ? "Save As:" : "Open:";
    ImGuiStyle& style = ImGui::GetStyle();
    ImGuiIO&    io    = ImGui::GetIO();
    ...
    float label_width =
      ImGui::CalcTextSize(label.c_str()).x + style.ItemSpacing.x;
    ...
    if (ImGui::InputTextWithHint("##FileNameInput",
                                 "Type a name...",
                                 &input_fn[0],
                                 256,
    ...
    // If Input Bar is edited show a list of files or dirs matching the input
    // text.
    if (ImGui::IsItemEdited() || ImGui::IsItemActivated()) {
        ...
        if (dialog_mode == DialogMode::OPEN ||
            dialog_mode == DialogMode::SAVE) {
            inputcb_filter_files.clear();
            for (int i = 0; i < subfiles.size(); i++) {
                if (ImStristr(
                      subfiles[i].name.c_str(), nullptr, input_fn, nullptr) !=
                    nullptr)
        ...
        // If dialog_mode == SELECT then filter from list of directories
            ...
            for (int i = 0; i < subdirs.size(); i++) {
                if (ImStristr(
                      subdirs[i].name.c_str(), nullptr, input_fn, nullptr) !=
                    nullptr)
    ...
    // Render a Drop Down of files/dirs (depending on mode) that have matching
    // characters as the input text only.
...
bool
ImGuiFileBrowser::renderButtonsAndCheckboxRegion()
{
    ImVec2      pw_size              = ImGui::GetWindowSize();
    ImGuiStyle& style                = ImGui::GetStyle();
    bool        show_error           = false;
    float       frame_height         = ImGui::GetFrameHeight();
    float       frame_height_spacing = ImGui::GetFrameHeightWithSpacing();
    float       opensave_btn_width =
      getButtonSize("Open")
        .x; // Since both Open/Save are 4 characters long, width gonna be same.
    float selcan_btn_width =
      getButtonSize("Cancel").x; // Since both Cacnel/Select have same number of
                                 // characters, so same width.
    float buttons_xpos;

    if (dialog_mode == DialogMode::SELECT)
        buttons_xpos = pw_size.x - opensave_btn_width -
                       (2.0f * selcan_btn_width) -
                       (2.0f * style.ItemSpacing.x) - style.WindowPadding.x;
    else
        buttons_xpos = pw_size.x - opensave_btn_width - selcan_btn_width -
                       style.ItemSpacing.x - style.WindowPadding.x;

    ImGui::SetCursorPosY(pw_size.y - frame_height_spacing -
                         style.WindowPadding.y);
    ...
    ImGui::Checkbox((show_marker) ? "##showHiddenFiles"
    ...
    // Render an Open Button (in OPEN/SELECT dialog_mode) or Open/Save depending
    // on what's selected in SAVE dialog_mode
    ImGui::SameLine();
    ImGui::SetCursorPosX(buttons_xpos);
    if (dialog_mode == DialogMode::SAVE) {
        // If directory selected and Input Text Bar doesn't have focus, render
        // Open Button
        ...
        if (ImGui::Button("Open")) {
            // It's possible for both to be true at once (user selected
...
bool
ImGuiFileBrowser::renderInputComboBox()
{
    bool        show_error     = false;
    ImGuiStyle& style          = ImGui::GetStyle();
    ...
    input_combobox_sz.y =
      std::min((inputcb_filter_files.size() + 1) * frame_height +
                 style.WindowPadding.y * 2.0f,
               8 * ImGui::GetFrameHeight() + style.WindowPadding.y * 2.0f);

    if (show_inputbar_combobox &&
        (ImGui::GetFocusScope() == focus_scope_id ||
        ...
        ImGui::BeginChild(
          "##InputBarComboBox", input_combobox_sz, true, popupFlags);

        ImVec2 listbox_size =
          input_combobox_sz - ImGui::GetStyle().WindowPadding * 2.0f;
        if (ImGui::ListBoxHeader("##InputBarComboBoxList", listbox_size)) {
            ...
            for (auto& element : inputcb_filter_files) {
                if (ImGui::Selectable(element.get().c_str(),
                                      false,
                    ...
                    if (element.get().size() > 256) {
                        error_title = "Error!";
                        error_msg =
                          "Selected File Name is longer than 256 characters.";
                        show_error = true;
                    } else {
...
void
ImGuiFileBrowser::renderExtBox()
{
    ImGui::PushItemWidth(ext_box_width);
    if (ImGui::BeginCombo("##FileTypes",
                          valid_exts[selected_ext_idx].c_str())) {
            ...
            if (ImGui::Selectable(valid_exts[i].c_str(),
                ...
                if (dialog_mode == DialogMode::SAVE) {
                    ...
                    if (idx == std::string::npos)
                        idx = strlen(input_fn);
                    for (int j = 0; j < valid_exts[selected_ext_idx].size();
                         j++)
...
bool
ImGuiFileBrowser::onNavigationButtonClick(int idx)
{
    std::string new_path = "";

    // First Button corresponds to virtual folder Computer which lists all
    // logical drives (hard disks and removables) and "/" on Unix
    if (idx == 0) {
#ifdef OSWIN
        ...
        if (idx == 1)
            ...
            // Start from i=1 since at 0 lies "MyComputer" which is only virtual
            // and shouldn't be read by readDIR
            for (int i = 1; i <= idx; i++)
                new_path += current_dirlist[i] + "/";
        }
#else
        // Since UNIX absolute paths start at "/", we handle this separately to
        // avoid adding a double slash at the beginning
    ...
    if (readDIR(new_path)) {
        current_dirlist.erase(current_dirlist.begin() + idx + 1,
                              current_dirlist.end());
...
bool
ImGuiFileBrowser::onDirClick(int idx)
{
    std::string name;
...
#ifdef OSWIN
    drives_shown =
      (current_dirlist.size() == 1 && current_dirlist.back() == "Computer");
#endif // OSWIN
    ...
    if (name == "..") {
        ...
        // Remember we displayed drives on Windows as *Local/Removable Disk: X*
        // hence we need last char only
        if (drives_shown)
...
bool
ImGuiFileBrowser::readDIR(std::string pathdir)
{
    DIR*           dir;
    struct dirent* ent;

    /* If the current directory doesn't exist, and we are opening the dialog for
     * the first time, reset to defaults to avoid looping of showing error
     * modal. An example case is when user closes the dialog in a folder. Then
     * deletes the folder outside. On reopening the dialog the current path
    ...
    if (dir != nullptr) {
#ifdef OSWIN
        // If we are on Windows and current path is relative then get absolute
        // path from dirent structure
        if (current_dirlist.empty() && pathdir == "./") {
            ...
            std::replace(
              current_directory.begin(), current_directory.end(), '\\', '/');
        ...
        // store all the files and directories within directory and clear
        // previous entries
        clearFileList();
...
// Somehow there is a '..' present in root directory in linux.
        ...
        error_title = "Error!";
        error_msg   = "Error opening directory! Make sure the directory exists "
                    "and you have the proper rights to access the directory.";
        return false;
    }
    return true;
}
...
void
ImGuiFileBrowser::filterFiles(int filter_mode)
{
    filter_dirty = false;
    ...
    if (filter_mode | FilterMode_Files) {
        ...
        for (size_t i = 0; i < subfiles.size(); ++i) {
            if (valid_exts[selected_ext_idx] == "*.*") {
                ...
                if (filter.PassFilter(subfiles[i].name.c_str()) &&
                    (ImStristr(subfiles[i].name.c_str(),
                               nullptr,
                               valid_exts[selected_ext_idx].c_str(),
                               nullptr)) != nullptr)
                    filtered_files.push_back(&subfiles[i]);
...
void
ImGuiFileBrowser::showHelpMarker(std::string desc)
{
    ImGui::TextDisabled("(?)");
...
void
ImGuiFileBrowser::showErrorModal()
{
    ...
    if (ImGui::BeginPopupModal(error_title.c_str(),
        ...
        ImGui::Separator();
        ImGui::SetCursorPosX(window_size.x / 2.0f -
                             getButtonSize("OK").x / 2.0f);
...
bool
ImGuiFileBrowser::showReplaceFileModal()
{
    ImVec2 window_size(250, 0);
    ImGui::SetNextWindowSize(window_size);
    bool ret_val = false;
    if (ImGui::BeginPopupModal(repfile_modal_id.c_str(),
                               nullptr,
                               ImGuiWindowFlags_NoScrollbar |
                                 ImGuiWindowFlags_NoResize)) {
        ...
        std::string text =
          "A file with the following filename already exists. Are you sure you "
          "want to replace the existing file?";
        ImGui::TextWrapped("%s", text.c_str());

        ImGui::Separator();

        float buttons_width = getButtonSize("Yes").x + getButtonSize("No").x +
                              ImGui::GetStyle().ItemSpacing.x;
        ImGui::SetCursorPosX(
          ImGui::GetCursorPosX() + ImGui::GetWindowWidth() / 2.0f -
...
void
ImGuiFileBrowser::showInvalidFileModal()
{
    ImGuiStyle& style = ImGui::GetStyle();
    std::string text =
      "Selected file either doesn't exist or is not supported. Please select a "
      "file with the following extensions...";
    ImVec2 text_size = ImGui::CalcTextSize(
      text.c_str(), nullptr, true, 350 - style.WindowPadding.x * 2.0f);
    ...
    if (ImGui::BeginPopupModal(invfile_modal_id.c_str(),
                               nullptr,
                               ImGuiWindowFlags_NoScrollbar |
                                 ImGuiWindowFlags_NoResize)) {
...
void
ImGuiFileBrowser::setValidExtTypes(const std::string& valid_types_string)
{
...
bool
ImGuiFileBrowser::validateFile()
{
    bool match = false;

    // If there is an item selected, check if the selected file name (the input
    // filename, in other words) matches the selection.
    if (selected_idx >= 0) {
        if (dialog_mode == DialogMode::SELECT)
    ...
    // If the input filename doesn't match we need to explicitly find the input
    // filename..
    if (!match) {
        if (dialog_mode == DialogMode::SELECT) {
            for (int i = 0; i < subdirs.size(); i++) {
    ...
    // If file doesn't match, return true on SAVE mode (since file doesn't
    // exist, hence can be saved directly) and return false on other modes
    // (since file doesn't exist so cant open/select)
    if (!match)
        return (dialog_mode == DialogMode::SAVE);

    // If file matches, return false on SAVE, we need to show a replace file
    // modal
    if (dialog_mode == DialogMode::SAVE)
        return false;
...
ImVec2
ImGuiFileBrowser::getButtonSize(std::string button_text)
{
    return (ImGui::CalcTextSize(button_text.c_str()) +
            ImGui::GetStyle().FramePadding * 2.0);
}

void
ImGuiFileBrowser::parsePathTabs(std::string path)
{
...
std::string
ImGuiFileBrowser::wStringToString(const wchar_t* wchar_arr)
{
    std::mbstate_t state = std::mbstate_t();

    // MinGW bug (patched in mingw-w64), wcsrtombs doesn't ignore length
    // parameter when dest = nullptr. Hence the large number.
...
bool
ImGuiFileBrowser::alphaSortComparator(const Info& a, const Info& b)
{
...
#ifdef OSWIN
bool
ImGuiFileBrowser::loadWindowsDrives()
{
    ...
    for (char* drv = nullptr; *temp != '\0'; temp++) {
        ...
        if (DRIVE_REMOVABLE == GetDriveTypeA(drv))
            subdirs.push_back(
              { "Removable Disk: " + std::string(1, drv[0]), false });
        else if (DRIVE_FIXED == GetDriveTypeA(drv))
            subdirs.push_back(
              { "Local Disk: " + std::string(1, drv[0]), false });
...
#ifndef OSWIN
void
ImGuiFileBrowser::initCurrentPath()
{
    ...
    // If PATH_MAX is defined deal with memory using new/delete. Else fallback
    // to malloc'ed memory from `realpath()`
    if (path_max_def)
