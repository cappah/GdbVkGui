...
static std::string
GetFieldValue(const char* line, const char* field, const char* delim_a, const char* delim_b)
{
...
ProcessGdbOutput(ParseType ptype)
{
	switch(ptype) {
		case ParseType::k_BreakPoint: {
			std::string b_show = GetFieldValue(s_output_text, "file", "\"", "\"");
			std::string b_file = GetFieldValue(s_output_text, "fullname", "\"", "\"");
			std::string b_line = GetFieldValue(s_output_text, "line", "\"", "\"");
			...
			TextEditor::Coordinates cpos(strtoul(b_line.c_str(), nullptr, 10), 0);
		...
		case ParseType::k_Frame: {
			std::string b_show = GetFieldValue(s_frame_watch, "file", "\"", "\"");
			std::string b_file = GetFieldValue(s_frame_watch, "fullname", "\"", "\"");
			std::string b_line = GetFieldValue(s_frame_watch, "line", "\"", "\"");

			memcpy(s_fname_show, b_show.c_str(), sizeof(s_fname_show));
			
			// open new file
			if (s_show_editor == false || (STR_EQ(b_file.c_str(), s_finfo.m_Name) == false)) {
				if (ReadFile(b_file.c_str(), &s_finfo)) {
					s_editor.SetText(s_finfo.m_Contents);
					s_editor.SetReadOnly(true);
					s_show_editor = true;
			...
			int32_t lnum = CLAMP(strtol(b_line.c_str(), nullptr, 10) - 1, 0, 10000000);
...
MainWindow(void)
{
	...
	if (s_FileDlg.showFileDialog("Open File", imgui_addons::ImGuiFileBrowser::DialogMode::OPEN, ImVec2(600, 300))) {
		snprintf(s_open_file, sizeof(s_open_file), "%s", s_FileDlg.selected_path.c_str());
	}
